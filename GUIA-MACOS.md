...
2. Baixe `claude-session-linker-macos.zip` ou o arquivo versionado com sufixo `-macos.zip`. Os dois arquivos da mesma release têm conteúdo idêntico; a versão fica em `.app/VERSION` e `RELEASE.json`.
...
Baixe o pacote macOS da última release, extraia em uma pasta nova e execute `00 - Setup Claude Session Linker.command`. Se quiser preservar backups antigos, copie somente `.app/backups` da instalação anterior. Não reutilize `.app/venv`, `.app/logs` nem arquivos de dados de contas e sessões.
